...
pub mod regs {
    ...
    impl Bufoutctrl {
        ...
        #[must_use]
        #[inline(always)]
        pub const fn peakdetthresana(&self) -> u8 {
            let val = (self.0 >> 12usize) & 0x0f;
            val as u8
        }
        #[doc = "Peak Detector Threshold for XOUT."]
        #[inline(always)]
        pub const fn set_peakdetthresana(&mut self, val: u8) {
    ...
    impl defmt::Format for Bufoutctrl {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Bufoutctrl {{ xoutbiasana: {=u8:?}, xoutcfana: {=u8:?}, xoutgmana: {=u8:?}, peakdetthresana: {=u8:?}, timeoutctune: {:?}, timeoutstartup: {:?}, minimumstartupdelay: {=bool:?} }}",
                self.xoutbiasana(),
                self.xoutcfana(),
                self.xoutgmana(),
