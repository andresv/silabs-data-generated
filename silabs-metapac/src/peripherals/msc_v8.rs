...
pub mod regs {
    ...
    impl defmt::Format for Status {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Status {{ busy: {=bool:?}, locked: {=bool:?}, invaddr: {=bool:?}, wdataready: {=bool:?}, eraseaborted: {=bool:?}, pending: {=bool:?}, timeout: {=bool:?}, reglock: {=bool:?}, pwron: {=bool:?}, wready: {=bool:?}, pwrupckbdfailcount: {=u8:?} }}",
                self.busy(),
                self.locked(),
                self.invaddr(),
                self.wdataready(),
    ...
    impl defmt::Format for Writecmd {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Writecmd {{ erasepage: {=bool:?}, writeend: {=bool:?}, eraseabort: {=bool:?}, erasemain0: {=bool:?}, clearwdata: {=bool:?} }}",
                self.erasepage(),
                self.writeend(),
                self.eraseabort(),
                self.erasemain0(),
    ...
    impl defmt::Format for Writectrl {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Writectrl {{ wren: {=bool:?}, irqeraseabort: {=bool:?}, lpwrite: {=bool:?} }}",
                self.wren(),
                self.irqeraseabort(),
                self.lpwrite()
            )
