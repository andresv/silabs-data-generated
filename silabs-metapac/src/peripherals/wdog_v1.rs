...
pub mod regs {
    ...
    impl Cfg {
        ...
        #[must_use]
        #[inline(always)]
        pub const fn em1run(&self) -> bool {
            let val = (self.0 >> 1usize) & 0x01;
            val != 0
        }
        #[doc = "EM1 Run."]
        #[inline(always)]
        pub const fn set_em1run(&mut self, val: bool) {
    ...
    impl defmt::Format for Cfg {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Cfg {{ clrsrc: {:?}, em1run: {=bool:?}, em2run: {=bool:?}, em3run: {=bool:?}, em4block: {=bool:?}, debugrun: {=bool:?}, wdogrstdis: {=bool:?}, prs0missrsten: {=bool:?}, prs1missrsten: {=bool:?}, persel: {:?}, warnsel: {:?}, winsel: {:?} }}",
                self.clrsrc(),
                self.em1run(),
                self.em2run(),
                self.em3run(),
